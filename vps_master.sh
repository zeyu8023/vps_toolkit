...
declare -A modules=(
  ...
  [7]="ssl_manager.sh:ssl_manager"
  [8]="install_tools.sh:install_tools"
  [9]="test_tools.sh:test_tools"
  [10]="log_tools.sh:log_tools"
  [11]="system_tools.sh:system_tools"
  [12]="update_center.sh:update_center"
)
...
while true; do
  ...
  printf "${YELLOW} 3.${RESET} Docker管理 🐳\n"
  printf "${YELLOW} 4.${RESET} 应用管理 📦\n"
  printf "${YELLOW} 5.${RESET} 内存管理 🧠\n"
  printf "${YELLOW} 6.${RESET} Swap管理 💾\n"
  printf "${YELLOW} 7.${RESET} SSL证书管理 🔐\n"
  printf "${YELLOW} 8.${RESET} 一键安装常用环境 🧰\n"
  printf "${YELLOW} 9.${RESET} 常用测试脚本功能 🧪\n"
  printf "${YELLOW}10.${RESET} 查看操作日志 📜\n"
  printf "${YELLOW}11.${RESET} 系统常用工具 🛠️\n"
  printf "${YELLOW}12.${RESET} 模块更新 🔄\n"
